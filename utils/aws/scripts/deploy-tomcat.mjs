...
import { loadAwsConfig, loadMagnoliaLicense } from './loadAwsConfig.mjs';
...
(async () => {
  try {
    ...
    // Load license from magnolia-license.properties (with env var override)
    const license = loadMagnoliaLicense();
    let magnoliaLicenseOwner = process.env.MAGNOLIA_LICENSE_OWNER || license.owner || '';
    let magnoliaLicenseKey = process.env.MAGNOLIA_LICENSE_KEY || license.key || '';
...
`;
    ...
    // Escape YAML strings properly (handle special characters)
    const escapeYamlString = (str) => {
      if (!str) return '';
      // Replace single quotes with double single quotes for YAML
      return str.replace(/'/g, "''");
    };

    // Build license properties for CATALINA_OPTS (will be passed to Ansible)
    const varsContent = `---
...
magnolia_license_owner: '${escapeYamlString(magnoliaLicenseOwner)}'
magnolia_license_key: '${escapeYamlString(magnoliaLicenseKey)}'
`;

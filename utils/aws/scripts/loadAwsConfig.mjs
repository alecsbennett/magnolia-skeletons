...
const licenseFile = join(awsDir, 'magnolia-license.properties');
...
/**
 * Load Magnolia license from properties file
 */
export function loadMagnoliaLicense() {
  let licenseProps = {};
  
  // Check if license file exists
  if (!existsSync(licenseFile)) {
    console.warn(`⚠️  Warning: Magnolia license file not found at ${licenseFile}`);
    console.warn(`   License will not be configured for AWS deployment.`);
    return { owner: '', key: '' };
  }
  
  try {
    const content = readFileSync(licenseFile, 'utf-8');
    licenseProps = parseProperties(content);
  } catch (error) {
    console.warn(`⚠️  Warning: Error reading Magnolia license file: ${error.message}`);
    return { owner: '', key: '' };
  }
  
  return {
    owner: licenseProps['magnolia.license.owner'] || '',
    key: licenseProps['magnolia.license.key'] || ''
  };
}
...
export function loadAwsConfig() {
  ...
  // Load Magnolia license from separate file
  ...
  const config = {
    ...
    magnoliaLicenseOwner: license.owner,
    magnoliaLicenseKey: license.key,
  };
